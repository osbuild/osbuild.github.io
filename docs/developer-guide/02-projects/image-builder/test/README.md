---
custom_edit_url: https://github.com/osbuild/image-builder/blob/main/test/README.md
---
...
[//]: # ( Rather change the source of this: https://github.com/osbuild/image-builder/blob/main/test/README.md )
-->
...
[./test/configs/](https://github.com/osbuild/image-builder/tree/main/test/configs/) contains configuration files for building images for testing. The files are used by the following tools:

- [./cmd/build](https://github.com/osbuild/image-builder/tree/main/cmd/build) takes a config file as argument to build an image.  For example:
```
...
- [./cmd/gen-manifests](https://github.com/osbuild/image-builder/tree/main/cmd/gen-manifests) generates manifests based on the configs specified in [./test/config-list.json](https://github.com/osbuild/image-builder/tree/main/test/config-list.json). The config list maps configuration files to image types, distributions, and architectures.  An empty list means it applies to all values.  Globs are supported.

The config list is also used in CI to dynamically generate test builds using the [./test/scripts/generate-build-config](https://github.com/osbuild/image-builder/tree/main/test/scripts/generate-build-config) and [./test/scripts/generate-ostree-build-config](https://github.com/osbuild/image-builder/tree/main/test/scripts/generate-ostree-build-config) scripts.

- [./test/data/repositories/](https://github.com/osbuild/image-builder/tree/main/test/data/repositories/) contains repository configurations for manifest generation ([./cmd/gen-manifests](https://github.com/osbuild/image-builder/tree/main/cmd/gen-manifests)) and image building ([./cmd/build](https://github.com/osbuild/image-builder/tree/main/cmd/build)).
...
- `Schutzfile` defines content sources and test variables:
    ...
    - `dependencies.osbuild.commit`: the version of osbuild to use, as a commit ID. This must be a commit that was successfully built in osbuild's CI, so that RPMs will be available. It is used by [./test/scripts/setup-osbuild-repo](https://github.com/osbuild/image-builder/tree/main/test/scripts/setup-osbuild-repo).
...
- Generates all the manifests for build config dependencies for a given distribution and architecture using the `./cmd/gen-manifests` tool.
  ...
  - For example [iot-ostree-pull-empty](https://github.com/osbuild/image-builder/tree/main/test/configs/iot-ostree-pull-empty.json) will cause a manifest to be generated for `iot-container` with the `empty` config for all distros.
